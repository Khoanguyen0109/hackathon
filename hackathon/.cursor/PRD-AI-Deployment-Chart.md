...
| Service | Purpose |
|---------|---------|
| **Node.js API** | Central backend — serves store/station/task/crew data, proxies AI requests, stores deployment charts. |
| **Python AI Service** | Receives store context + external factors, constructs prompts, calls Ollama, parses and returns structured staffing suggestions. |
| **SQLite Database** | Stores all application data (stores, stations, tasks, crew, deployment charts). Used for hackathon simplicity. |
...
- The Node.js API provides health-check endpoints for each service (API, AI service, database).
...
| **Python environment** | Python 3.10+ required for the AI service. |
| **Node.js environment** | Node.js 20+ for the API layer. |
